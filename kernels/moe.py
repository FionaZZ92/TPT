...
# This is vllm original triton op
@triton.jit
...
def get_moe_configs(
    ...
    if spec and spec.origin:
        user_defined_config_folder = os.path.dirname(spec.origin)+"/model_executor/layers/fused_moe/configs" #replace envs.VLLM_TUNED_CONFIG_FOLDER
    else:
        user_defined_config_folder = None
...
def fused_experts_impl(
    ...
    CHUNK_SIZE = 16 * 1024 #from vllm, flashinfer 8192?
    ...
    for chunk in range((num_tokens // CHUNK_SIZE) + 1):
        ...
        #将输入 Token 按专家（Expert）分组对齐，为后续专家计算做准备
...
def fused_topk(
    ...
    #topk_weights, topk_ids = ops.topk_softmax(topk_weights, topk_ids, token_expert_indices, gating_output, renormalize)
    try:
        topk_weights, topk_ids = aiter.topk_softmax(topk_weights, topk_ids, token_expert_indices, gating_output, renormalize)
    except TypeError:
        print("topk_softmax函数返回 None")
